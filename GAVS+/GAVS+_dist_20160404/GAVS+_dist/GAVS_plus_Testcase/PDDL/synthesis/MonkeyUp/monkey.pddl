(define (domain monkey)	       
  ...
  (:action CLIMB
	     :parameters (?x)
	     :precondition (and (P0TRAN) (location ?x) (at box ?x) (at monkey ?x))
	     :effect (and (not (P0TRAN)) (onbox ?x) (not (on-floor))))
  
  (:action PUSH-BOX
	     :parameters (?x ?y)
	     :precondition (and (P0TRAN) (location ?x) (location ?y) (at box ?y) (at monkey ?y) 
				 (on-floor))
	     :effect (and (not (P0TRAN)) (at monkey ?x) (not (at monkey ?y))
  ...
  (:action GET-KNIFE
	     :parameters (?y)
	     :precondition (and (P0TRAN) (location ?y) (at knife ?y) (at monkey ?y))
  ...
  (:action BLOW
	      :parameters (?x ?y)
	      :precondition (and (not (P0TRAN)) (not (blow))  (location ?x) (location ?y) (at bananas ?y))
	      :effect (and  (P0TRAN) (at bananas ?x) (not (at bananas ?y)) (blow)) )
