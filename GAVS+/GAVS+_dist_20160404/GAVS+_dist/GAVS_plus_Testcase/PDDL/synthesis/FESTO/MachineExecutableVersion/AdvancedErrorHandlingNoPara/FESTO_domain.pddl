;;
;; Author: Chih-Hong Cheng, Michael Geisinger @ fortiss (An Institute of TU Muenchen)
;;
...
(define (domain FESTO_domain)
	...
	(:action belt-move-b
		:parameters  (?obj - unit ?from - beltposition ?to - beltposition)
	...
	(:action belt-move-r
		:parameters  (?obj - unit ?from - robotposition  ?to - beltposition)
		:precondition (and (P0TRAN) (belt-connected ?from ?to) (at ?obj ?from) (not (occupied ?to)) (not (in-robot ?from)))
		:effect (and  (P0TRAN) (not (occupied ?from)) (not (at ?obj ?from)) (at ?obj ?to) (occupied ?to) 
			)
	)
