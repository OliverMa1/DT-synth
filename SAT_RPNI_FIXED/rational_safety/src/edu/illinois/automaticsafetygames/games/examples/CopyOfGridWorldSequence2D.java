...
public class CopyOfGridWorldSequence2D implements IGame {
	...
	public CopyOfGridWorldSequence2D(boolean[][] world) {
		...
		for (int i = 0; i < n; ++i) {
			...
			if (world[i] == null || world[i].length == 0) {
				throw new IllegalArgumentException("world contains null row");
			}
			...
			if (i == 0) {
				...
				if (world[i].length != m) {
					throw new IllegalArgumentException(
							"world contains rows of different length");
				}
			}
	...
	private void populateArray(boolean[][] world) {
		...
		for (int i1 = 0; i1 < n - 1; ++i1) {
			for (int j1 = 0; j1 < m; ++j1) {
				for (int i2 = 0; i2 < n; ++i2) {
					for (int j2 = 0; j2 < m; ++j2) {
						if (!world[i1 + 1][j1]) {
							// System.out.println("Yeah way (" + i1 + ", " + j1
							// + ")");
							states[0][i1][j1][i2][j2]
									.addTransition(new Transition('\u0000',
											states[1][i1 + 1][j1][i2][j2]));
						} else {
							// System.out.println("no way (" + i1 + ", " + j1 +
							// ")");
						}
					}
				}
	...
	public static void main(String[] args) {
		...
		boolean[][] world = { { false, false, false }, { true, true, true } };

		CopyOfGridWorldSequence2D game = new CopyOfGridWorldSequence2D(world);

		System.out.println(Tools.automatonToDot(game.aut));
